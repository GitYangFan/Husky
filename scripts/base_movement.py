...
def straight(linear_speed, distance):
    ...
    while not rospy.is_shutdown() or (rospy.Time.now() - start_time).to_sec() >= (distance/linear_speed):
...
def rotate(angular_speed, angle):
    ...
    start_time = rospy.Time.now()
    while not rospy.is_shutdown() or (rospy.Time.now() - start_time).to_sec() >= (angle/angular_speed):
        pub.publish(move_cmd)
        rate.sleep()
